...
void GazeboRosBlockLaser::OnStats( const boost::shared_ptr<msgs::WorldStatistics const> &_msg)
{
  ...
  pose.Set(ignition::math::Vector3d(0, 0, 0), ignition::math::Quaterniond(0.0, 0.5*sin(0.01*this->sim_time_.Double()), 0, 0));
  gzdbg << "plugin simTime [" << this->sim_time_.Double() << "] update pose [" << pose << "]\n";
}
}
